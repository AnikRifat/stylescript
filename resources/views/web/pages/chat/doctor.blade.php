            ...
            <div class="rounded-16 bg-white -dark-bg-dark-1 shadow-4 h-100">
                ...
                <div class="py-30 px-30">
                    <div class="y-gap-30">
                        ...
                        @endphp --}}
                        @php
                        $customer_ids = App\Models\Order::where('item_id' , $item->id)->pluck('user_id');
                        $customers =App\Models\User::whereIn('id' , $customer_ids)->get();

                        @endphp
                        @foreach ($customers as $customerinfo)
                        <div class="d-flex justify-between">

                            <a href="{{ route('chat.show.doctor',[$customerinfo->id,$item->id]) }}">
                                <div class="d-flex items-center">

                                    <div class="shrink-0">
                                        <img src="{{ asset('uploads') }}/customers/{{ $customerinfo->customer->image }}"
                                          alt="image" class="size-50">
                                    </div>
                                    <div class="ml-10">
                                        <div class="text-14 lh-11 mt-5">{{ $customerinfo->name }}</div>
                                    </div>
                        ...
                        <div class="col-xl-7 col-lg-10">
                            <div class="d-flex items-center">
                                <div class="shrink-0">
                                    ...
                                    $customerInfo = App\Models\User::find($chat->sender_id);

                                    @endphp
                                    <img src="{{ asset('') }}uploads/customers/{{ $customerInfo->customer->image }}"
                                      alt="image" class="size-50">
                                </div>
                                <div class="lh-11 fw-500 text-dark-1 ml-10">{{ $customerInfo->name }}</div>
